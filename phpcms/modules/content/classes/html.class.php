...
class html {
	...
	public function show($file, $data = '', $array_merge = 1,$action = 'add',$upgrade = 0) {
		...
		$qsw521_template_cache_app 		= $redis->qsw521_template_cache_app(); //页面缓存
		$redis->qsw_html_set(array('modelid'=>$modelid,'catid'=>$catid,'id'=>$id));
		if($paginationtype!=0) {
			//手动分页
			...
			if($CONTENT_POS !== false) {
				...
				foreach ($pageurls as $page=>$urls) {
					...
					$redis->qsw521_template_cache_app(2,$qsw521_template_cache_app);
				}
				return true;
			}
		}
		...
		ob_start();
		include template('content', $template);
		if(BAIDU_SITEMAP_TYPE){
			$redis->baidu_sitemap($file_url); // 百度sitemap
	...
	public function category($catid, $page = 0) {
		...
		if($type==0) {
			$template = $setting['category_template_app'] ? $setting['category_template_app'] : 'category';
			$template_list = $setting['list_template_app'] ? $setting['list_template_app'] : 'list';
	...
	*/
	...
	public function category_type( $type_arr,$page = 0,$file,$template='show_ctwh') {
		pc_base::load_sys_class('redis','',0);
		$redis 	= new RedisWith(1);
		$qsw521_with_set_arr 		= $redis->qsw521_with_set('',3);
		if($qsw521_with_set_arr){
			$qsw521_with_set_arr_type = 1;
		}
		$SEO = seo($this->siteid);
		$siteid = $this->siteid;
		$CATEGORYS = $this->categorys;
		$style = $this->sitelist[$siteid]['default_style'];
		ob_start();
		include template('content',$template,$style);
		if(!$qsw521_with_set_arr_type){
			$redis->qsw521_with_set($qsw521_with_set_arr,4);
		}
		return $this->createhtml($file);
