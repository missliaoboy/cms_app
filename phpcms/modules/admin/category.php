...
class category extends admin {
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$msetting = $_POST['msetting'];
			$_POST['info']['msetting'] = array2string($msetting);
			...
			if(isset($_GET['parentid'])) {
				...
				$msetting = string2array($msetting);
			}
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['template_child']){
                ...
                $idstr = $this->get_arrchildid($catid);
                 if(!empty($idstr)){
	                $sql = "select catid,msetting from phpcms_category where catid in($idstr)";
	                $this->db->query($sql);
	                $arr = $this->db->fetch_array();
	                 if(!empty($arr)){
                        foreach ($arr as $v){
                            $new_setting = array2string(
							array_merge(string2array($v['msetting']), array('category_template_app' => $_POST['msetting']['category_template_app'],'list_template_app' =>  $_POST['msetting']['list_template_app'],'show_template_app' =>  $_POST['msetting']['show_template_app'])
                                )
                            );
                            $this->db->update(array('msetting'=>$new_setting), 'catid='.$v['catid']);
                        }
	                }                                
                }
			}
	...
	*/
	...
	public function public_tpl_file_list() {
		...
		if ($catid) {
			...
			$cat = $cat[$catid];
			$cat['msetting'] = string2array($cat['msetting']);
			$cat2['setting'] = string2array($cat['setting']);
		}
		pc_base::load_sys_class('form','',0);
		if($_GET['type']==1) {
			$html = array('page_template'=>form::select_template($style, 'content',(isset($cat2['setting']['page_template']) && !empty($cat2['setting']['page_template']) ? $cat2['setting']['page_template'] : 'category'),'name="setting'.$batch_str.'[page_template]"','page'));
		} else {
			$html = array(
				'category_template'=> form::select_template($style, 'content',(isset($cat['msetting']['category_template_app']) && !empty($cat['msetting']['category_template_app']) ? $cat['msetting']['category_template_app'] : 'category'),'name="msetting'.$batch_str.'[category_template_app]"','category'), 
				'list_template'=>form::select_template($style, 'content',(isset($cat['msetting']['list_template_app']) && !empty($cat['msetting']['list_template_app']) ? $cat['msetting']['list_template_app'] : 'list'),'name="msetting'.$batch_str.'[list_template_app]"','list'),
				'show_template'=>form::select_template($style, 'content',(isset($cat['msetting']['show_template_app']) && !empty($cat['msetting']['show_template_app']) ? $cat['msetting']['show_template_app'] : 'show'),'name="msetting'.$batch_str.'[show_template_app]"','show')
			);
		}
		if ($_GET['module']) {
			unset($html);
	...
	public function public_change_tpl() {
		...
		if($_GET['modelid']) {
			...
			$category_template = $models[$modelid]['category_template_app'];
			$list_template = $models[$modelid]['list_template_app'];
			$show_template = $models[$modelid]['show_template_app'];
			$html = array(
				'template_list'=> $style, 
				'category_template'=> form::select_template($style, 'content',$category_template,'name="msetting[category_template_app]"','category'), 
				'list_template'=>form::select_template($style, 'content',$list_template,'name="msetting[list_template_app]"','list'),
				'show_template'=>form::select_template($style, 'content',$show_template,'name="msetting[show_template_app]"','show')
			);
			if (CHARSET == 'gbk') {
				$html = array_iconv($html, 'gbk', 'utf-8');
			}
	...
	public function batch_edit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			foreach ($infos as $catid=>$info) {
				$msetting = string2array($categorys[$catid]['msetting']);
				foreach($post_setting[$catid] as $_k=>$_setting) {
					$msetting[$_k] = $_setting;
				}
				//内容生成静态配置
				$info['msetting'] = array2string($msetting);
