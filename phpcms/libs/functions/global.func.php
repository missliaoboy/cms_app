...
function create_randomstr($lenth = 6) {
	return random($lenth, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz');
}
...
{
	if(!$content)return '';
	$content 	= preg_replace('/<img[^>]+?src=((?![\"|\'])[\S\s]+?) [^>]+?>/i', "<img src='$1'>", $content);
	$content 	= preg_replace('/<((?![a|img])([^>]+?)) [^>]*>/i', '<$1>', $content);   
	$content 	= preg_replace('/div/i', 'p', $content);
	$content 	= preg_replace('/<a[^>]*>|<font[^>]*>|<\/font>|　　|	| |<\/a>|<div[^>]*>|<\/div>|<tbody[^>]*>|<\/tbody>|<table[^>]*>|<\/table>|<body[^>]*>|<\/body>|<script[^>]*>([^>]*)<\/script>|<script[^>]*><\/script>|<td[^>]*>|<\/td>|<tr[^>]*>|<\/tr>|<bb[^>]*>|<\/bb>|<ul[^>]*>|<\/ul>|<li[^>]*>|<\/li>|<iframe[^>]*>|<\/iframe>|<!--[\S\s]+?-->|<p><\/p>/i', '', $content);
	$content 	= preg_replace("/<span><\/span>|<font><\/font>|<font>\s+<\/font>/i", '', $content);
	$content 	= preg_replace("/(.*)<br[^>]*>/i", "<p>$1</p>", $content);
	$content 	= preg_replace("/(\r\n){2,}/i", "\r\n", $content);
	$content 	= preg_replace("/<br[^>]*>|<p>\s*[\r|\n]*\s*<\/p>|<p><\/p>/i", "", $content);
	$content 	= preg_replace("/<p>[\r|\n]<p>/i", "<p>", $content);
	$content 	= preg_replace("/<p>\n\s*\r<\/p>/i", "", $content);
	$content 	= preg_replace('/<img[^>]*src=([\'|\"| ])([^>]+?)([\'|\"| ])[^>]*>/i',"<img src='".$url."$2' alt='".$title."' />",$content);
...
function collect_curl_collect($url)
{
	...
	if($encode != 'UTF-8'){
		if($encode == 'CP936'){
			$content = iconv("CP936","UTF-8//IGNORE",$content);//碰到这种编码情况，我先暂时加上这种处理方式，以后碰到问题再做调试
		}else{
...
function pages_type_list($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 9) {
	...
	$prefix_url = $_POST['level'] == 1 ? $type_deam[$_POST['id']]['prefix_url'] : '';
	if(!$prefix_url)$prefix_url 	= $_POST['level'] == 2 && $prefix_url == '' ? $type[$_POST['id']]['prefix_url'] : '';
	$urlrule 	= '/' . trim($prefix_url,'/') . '/list_{$page}.html';
	$multipage  = '';
	if($num > $perpage) {
		$page = $setpages+1;
		$offset = ceil($setpages/2-1);
...
function Hanzi2PinYin2($value,$type=false){
	...
	foreach ($arr as $key => $str) {
	    ...
	    for($i=0;$i<$slen;$i++){
	        ...
	        if(ord($str1)>0x80){
	            if(isset($pinyins2[$str1])){
	                $res .= '<rb>'.$str1.'</rb><rt>'.$pinyins2[$str1].'</rt>'."\r\n";
	            }else{
	            	$res .= '<rb class="sign">'.$str1.'</rb>'."\r\n";
	            }
	        }else{
	            $res .= '<rb class="sign">'.$str1.'</rb>'."\r\n";
	        } 
 ...
 */
function string_split($content,$type=false)
{
	...
	if($num == 1)return Hanzi2PinYin(mb_substr($content, $i,1,'utf-8'));
	for( $i = 0 ; $i < $num ; $i++ ){
